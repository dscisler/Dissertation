...
for (i in 1:length(file_names_listed_EXP1notEnjoyRaw)) {
  ...
  test1 <- test[test$trialtype == 'Go',]
  test2 <- test[test$trialtype == 'NoGo',]
  Analysis2[nrow(Analysis2) + 1,] <- c(unique(procPart$subject), paste('not enjoy'), test1$Means, mean(procPart$RT, na.rm = TRUE), test2$Means, sum(procPart$hit), sum(procPart$miss), sum(procPart$fa), sum(procPart$cr), playlist$playlistSel)
