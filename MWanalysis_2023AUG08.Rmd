...
for (i in 1:length(file_names_listed_EXP1EnjoyRaw)) {
  ...
  Analysis[nrow(Analysis) + 1,] <- c(unique(New_part$subject), paste('enjoy'), test1$Means, mean(New_part$RT, na.rm = TRUE), test2$Means, sum(New_part$hit), sum(New_part$miss), sum(New_part$fa), sum(New_part$cr), playlist$playlistSel)
