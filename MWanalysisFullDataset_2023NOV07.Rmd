...
##Creates list of file names
datalist = list()
file_names_listed_Exp3EnjoyRaw <- list.files(path = "C:/Users/Cisler/Desktop/Data/For Analysis/Exp3Enjoy/SART/Raw", full.names = TRUE,
                                             pattern = "*.iqdat")

for (i in 1:length(file_names_listed_Exp3EnjoyRaw)) {
  
  ####Loading and processing data####
  #part <- read_xlsx(file_names_listed_EXP1Enjoy[i])
  part <- read.table(file_names_listed_Exp3EnjoyRaw[i],
                     sep='\t', header=TRUE)
...
AnalysisExp3enjoy <- bind_rows(datalist)
...
##Create list of file names in Enjoy Survey Directory
file_names_listed_Exp3EnjoySurvey <- list.files(path = "C:/Users/Cisler/Desktop/Data/For Analysis/Exp3Enjoy/SART/Survey", full.names = TRUE,
                                                pattern = "*.iqdat")

mwDatlist <- list()
##Auto Process Enjoy Survey Data
for (j in 1:length(file_names_listed_Exp3EnjoySurvey)) {
  AnalysisExp3 <- data.frame(matrix(ncol = 50, nrow = 1))

colnames(AnalysisExp3) <- c("build", "computer.platform", "date", "time", "subject", "group", "sessionid", "blockcode", "blocknum", "trialcode", "trialnum", "trialcount", "digitpresentationtime", "maskpresentationtime", "trialtype", "digit", "fontsize", "response", "correct", "RT", "latency", "latencytype", "responsetype", "count_anticipatory", "correctsuppressions", "incorrectsuppressions", "count_NoGo", "count_Go", "count_validgo", "countprobes", "radiobuttons.difficulty.response", "radiobuttons.interest.response", "slider.GTQ.response", "slider.paid.response", "radiobuttons.PlaylistPick.response", "rock", "sc", "sccount", "radiobuttons.gender.response", "radiobuttons.handedness.response", "radiobuttons.meds.response", "radiobuttons.diag.response", "radiobuttons.vision.response", "radiobuttons.hearing.response", "video.soundtest.currentitem", "screenCred",  "playlist",  "exp", "cond", "filt")

  
  part_survey <- read.table(file_names_listed_Exp3EnjoySurvey[j],
                            sep='\t', header=TRUE)
  mw_resp <- subset(part_survey, select = c(date, time, group, subject, session, build, probe_response, probe_latency, probe2_response, probe2_latency, MWcatch_response, MWcatch_latency, MWonTask_response, MWonTask_latency, MWother_response, MWother_latency))
  gatherCol <- c('probe_response', 'probe2_response', 'MWcatch_response', 'MWonTask_response', 'MWother_response')
  #gatherCol2 <- c('probe_latency', 'probe2_latency', 'MWcatch_latency', 'MWonTask_latency', 'MWother_latency')
  ...
  #This copies the data from mw_respLong over to AnalysisExp3 dataframe.  Trial Count will be set to 1201, so after I rbind it to the dataset with the raw data, I can arrange it by subject number and trial count.  
  AnalysisExp3[nrow(AnalysisExp3) + length(mw_respLong$subject) -1, ] <- AnalysisExp3
  AnalysisExp3$subject <- ifelse(is.na(AnalysisExp3$subject), mw_respLong$subject,NA)
  AnalysisExp3$group <- ifelse(is.na(AnalysisExp3$group), mw_respLong$group,NA)
  AnalysisExp3$trialcode <- ifelse(is.na(AnalysisExp3$trialcode), mw_respLong$trialcode,NA)
  AnalysisExp3$response <- ifelse(is.na(AnalysisExp3$response), mw_respLong$response,NA)
  AnalysisExp3$latency <- ifelse(is.na(AnalysisExp3$latency), mw_respLong$latency,NA)
  AnalysisExp3$trialcount <- ifelse(is.na(AnalysisExp3$trialcount), 1201,NA)
  
  mwDatlist[[j]] <- AnalysisExp3
    #Removes unwanted columns from survey data
  part_survey <- subset(part_survey, select = c(subject, mentaldemand_response, physicaldemand_response, temporaldemand_response,
                                                performance_response, effort_response, frustration_response, difficulty_response,
                                                GTQ_response, interest_response, paid_response))
...
MWpostprobeEnjoy <- bind_rows(mwDatlist)
MWpostprobeEnjoy$exp <- 'exp3'
MWpostprobeEnjoy$cond <- 'enjoy'

Exp3BindEnjoy <- rbind(AnalysisExp3enjoy, MWpostprobeEnjoy)
Exp3BindEnjoyArranged <- Exp3BindEnjoy %>% arrange(subject)

#75408185

##Append Survey Data to Task Data
Exp3BindEnjoyArranged$subject <- as.character(Exp3BindEnjoyArranged$subject)
Exp3BindEnjoyArranged <- full_join(Exp3BindEnjoyArranged, New_part_survey, by = 'subject')

```


```{r processing exp3 not enjoy cond}
#This one is going to be a little bit different.  I need to create a blank dataframe to add in the mind wandering probe then rbind it to the raw dataset.
#trialcode is probe, trialtype is probe, digit is NA, response is response, correct is 1, RT is NA, latency is latency, latencytype is NA
#Follow-up page: trialcode is probe2, trialtype is 
...
##Creates list of file names
datalist = list()
file_names_listed_Exp3notEnjoyRaw <- list.files(path = "C:/Users/Cisler/Desktop/Data/For Analysis/Exp3NotEnjoy/SART/Raw", full.names = TRUE,
                                                pattern = "*.iqdat")


for (i in 1:length(file_names_listed_Exp3notEnjoyRaw)) {
  
  ####Loading and processing data####
  #part <- read_xlsx(file_names_listed_EXP1NotEnjoy[i])
  part <- read.table(file_names_listed_Exp3notEnjoyRaw[i],
                     sep='\t', header=TRUE)
...
AnalysisExp3NotEnjoy <- bind_rows(datalist)
...
##Create list of file names in NotEnjoy Survey Directory
file_names_listed_Exp3NotEnjoySurvey <- list.files(path = "C:/Users/Cisler/Desktop/Data/For Analysis/Exp3NotEnjoy/SART/Survey", full.names = TRUE,
                                                pattern = "*.iqdat")

mwDatlist <- list()
##Auto Process NotEnjoy Survey Data
for (j in 1:length(file_names_listed_Exp3NotEnjoySurvey)) {
  AnalysisExp3 <- data.frame(matrix(ncol = 50, nrow = 1))

colnames(AnalysisExp3) <- c("build", "computer.platform", "date", "time", "subject", "group", "sessionid", "blockcode", "blocknum", "trialcode", "trialnum", "trialcount", "digitpresentationtime", "maskpresentationtime", "trialtype", "digit", "fontsize", "response", "correct", "RT", "latency", "latencytype", "responsetype", "count_anticipatory", "correctsuppressions", "incorrectsuppressions", "count_NoGo", "count_Go", "count_validgo", "countprobes", "radiobuttons.difficulty.response", "radiobuttons.interest.response", "slider.GTQ.response", "slider.paid.response", "radiobuttons.PlaylistPick.response", "rock", "sc", "sccount", "radiobuttons.gender.response", "radiobuttons.handedness.response", "radiobuttons.meds.response", "radiobuttons.diag.response", "radiobuttons.vision.response", "radiobuttons.hearing.response", "video.soundtest.currentitem", "screenCred",  "playlist",  "exp", "cond", "filt")

  
  part_survey <- read.table(file_names_listed_Exp3NotEnjoySurvey[j],
                            sep='\t', header=TRUE)
  mw_resp <- subset(part_survey, select = c(date, time, group, subject, session, build, probe_response, probe_latency, probe2_response, probe2_latency, MWcatch_response, MWcatch_latency, MWonTask_response, MWonTask_latency, MWother_response, MWother_latency))
  gatherCol <- c('probe_response', 'probe2_response', 'MWcatch_response', 'MWonTask_response', 'MWother_response')
  #gatherCol2 <- c('probe_latency', 'probe2_latency', 'MWcatch_latency', 'MWonTask_latency', 'MWother_latency')
...
MWpostprobeNotEnjoy <- bind_rows(mwDatlist)
MWpostprobeNotEnjoy$exp <- 'exp3'
MWpostprobeNotEnjoy$cond <- 'NotEnjoy'

Exp3BindNotEnjoy <- rbind(AnalysisExp3NotEnjoy, MWpostprobeNotEnjoy)
Exp3BindNotEnjoyArranged <- Exp3BindNotEnjoy %>% arrange(subject)

#75408185

##Append Survey Data to Task Data
Exp3BindNotEnjoyArranged$subject <- as.character(Exp3BindNotEnjoyArranged$subject)
Exp3BindNotEnjoyArranged <- full_join(Exp3BindNotEnjoyArranged, New_part_survey, by = 'subject')


AnalysisFull <- rbind(AnalysisExp1enjoy, AnalysisExp1NotEnjoy, Exp2BindEnjoyArranged, Exp2BindNotEnjoyArranged, Exp3BindEnjoyArranged, Exp3BindNotEnjoyArranged)

write_rds(AnalysisFull, 'C:/Users/Cisler/Desktop/Data/FullDataset.rds')
write.csv(AnalysisFull, 'C:/Users/Cisler/Desktop/Data/FullDataset.csv')
```