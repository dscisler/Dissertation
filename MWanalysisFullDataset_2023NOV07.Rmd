...
Analysisenjoy <- bind_rows(datalist)
...
Analysisenjoy$subject <- as.character(Analysisenjoy$subject)
Analysisenjoy <- full_join(Analysisenjoy, New_part_survey, by = 'subject')
...
AnalysisNotEnjoy <- bind_rows(datalist)
...
##Append Survey Data to Task Data
AnalysisNotEnjoy$subject <- as.character(AnalysisNotEnjoy$subject)
AnalysisNotEnjoy <- full_join(AnalysisNotEnjoy, New_part_survey, by = 'subject')

AnalysisFull <- rbind(Analysis, AnalysisNotEnjoy)
```
...
Analysisexp2 <- data.frame(matrix(ncol = 50, nrow = 0))
...
for (j in 1:length(file_names_listed_EXP1EnjoySurvey)) {
  ...
  gatherCol <- c('date', 'time', 'group', 'subject', 'session', 'build', 'probe_response', 'probe2_response', 'MWcatch_response', 'MWonTask_response', 'MWother_response')
  ...
  mw_respLong <- gather(mw_resp, trialtype, response, gatherCol)
  mw_respLong$latency <- ifelse(mw_respLong$trialtype == 'probe_response', mw_respLong$probe_latency, mw_respLong$probe2_latency)
  #Analysisexp2 <- rbind(mw_respLong)
  
  tmp <- rbind(A)
  
  Analysisexp2$trialcode <- mw_respLong$trialtype
  Analysisexp2$response <- mw_respLong$response
  Analysisexp2$latency <- mw_respLong$latency
