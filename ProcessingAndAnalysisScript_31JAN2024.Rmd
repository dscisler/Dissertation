...
EXP1df$trialNum <- as.factor(EXP1df$trialnum)

# Probelm <- lmer(response~time.x + (1|subject), data = EXP1df)
# summary(RTlm)
# RTlmPseudoR2 <- r.squaredGLMM(RTlm)
# 
...
EXP2MWcatch <- EXP2df[!duplicated(EXP2df), ]
EXP2MWcatch <- EXP2MWcatch %>% dplyr::select(subject, MWcatch_response, MWonTask_response, MWother_response)

MergedMWcatch <- rbind(EXP1MWcatchTrialsAverages, EXP2MWcatch)
...
MWassign <- left_join(MWassign, MergedMWcatch, by = 'subject')
MWassign$MWcatch_response[is.na(MWassign$MWcatch_response)] <- 0

MWassign$MWer_final <- ifelse(MWassign$MWer == "Yes" & MWassign$MWcatch_response >=51, "MW", "NMW")
MWassign <- MWassign %>% 
  rename(MWer_pre = MWer,
         MWer = MWer_final)

MWassign <- MWassign %>% dplyr::select(subject, new_exp_column, new_cond_column, probe_response, paid_response, mturk_sona, MWer)
...
exp1exp2Analysis2$MWer <- relevel(exp1exp2Analysis2$MWer, ref = "NMW")
...
SARTrtFilt <- SARTrtFilt %>% filter(trialtype == "Go")
...
Missmean <- aggregate(miss ~ subject + new_exp_column + new_cond_column, SARTmissFilt, mean)
...
# summary(RTlm)
SARTrtSubCheck <- SARTrtFilt[!duplicated(SARTrtFilt$subject), ]
table(SARTrtSubCheck$new_exp_column, SARTrtSubCheck$new_cond_column)
...
RTbyMWExp1 <- ggplot(participant_avg_latencyExp1, aes(x = mturk_sona, y = avg_latency, fill = mturk_sona)) + 
  ...
  guides(fill = FALSE)  + coord_flip()  # Explicitly remove the legend
...
participant_avg_latencyExp1_pres <- SARTrtFilt %>%
  group_by(subject, mturk_sona, new_exp_column) %>%
  summarize(avg_latency = mean(latency),
            sd_latency = sd(latency))

study1RT_sample_sizes <- SARTrtFilt %>% 
  group_by(mturk_sona, new_exp_column) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_latency = max(latency, na.rm = TRUE))


RTbyMWExp1 <- ggplot(participant_avg_latencyExp1_pres, aes(x = mturk_sona, y = avg_latency, fill = new_exp_column)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Reaction Time of Go Trials Between MTurk and SONA System Participants \nby Probe Condition", 
       x = "Participant Pool", 
       y = "Average Reaction Time of Go Trials",
  ...
  scale_fill_manual(values = c("EXP1" = "blue", "EXP2" = "red"),
                    labels = c("EXP1" = "Interrupting Probe", "EXP2" = "Post Probe")) + coord_flip()
...
RTbyMWExp1 + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study1RT_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 900 + 0.5, 
                x = mturk_sona,
                color = new_exp_column),  # Use Probe for the interaction and color
            position = position_dodge(width = 0.9),  
            hjust = 0, vjust = -0.5, size = 5, color = "black", fontface = "bold", show.legend = FALSE)
...
ACClmPseudoR2 <- r.squaredGLMM(ACClm)
...
#For presentation
participant_avg_accExp1_pres <- SARTaccFilt %>%
  group_by(subject, mturk_sona, new_exp_column) %>%
  summarize(avg_correct = mean(correct),
            sd_correct = sd(correct))

study1ACC_sample_sizes <- SARTaccFilt %>% 
  group_by(mturk_sona, new_exp_column) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_acc = max(correct, na.rm = TRUE))

ACCbyMWExp1 <- ggplot(participant_avg_accExp1_pres, aes(x = mturk_sona, y = avg_correct, fill = new_exp_column)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Accuracy of No Go Trials Between MTurk and SONA System Participants \nby Probe Condition", 
       x = "Participant Pool", 
       y = "Average Accuracy of No Go Trials",
  ...
  scale_x_discrete(labels = c("MT" = "MTurk", "SS" = "SONA Systems")) +
  scale_fill_manual(values = c("EXP1" = "blue", "EXP2" = "red"),
                    labels = c("EXP1" = "Interrupting Probe", "EXP2" = "Post Probe")) + coord_flip()  # Explicitly remove the legend
...
ACCbyMWExp1 + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study1ACC_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 0.8 + 0.25, 
                x = mturk_sona,
                color = new_exp_column),  # Use Probe for the interaction and color
...
FAlmPseudoR2 <- r.squaredGLMM(FAlm)
...
FAbyMWExp1 <- ggplot(participant_avg_faExp1, aes(x = new_exp_column, y = avg_FA, fill = new_exp_column)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Errors of Omission of Go Trials By Probe Condition", 
       x = "Probe Condition", 
       y = "Average Errors of Omission of Go Trials",
       fill = "Switch") +
...
Misslm <- lmer(miss ~ mturk_sona*new_exp_column*new_cond_column + (1|subject), data = SARTmissFilt)
MisslmConfInt <- confint(Misslm)
MissLMERresults <- summary(Misslm)
MisslmPseudoR2 <- r.squaredGLMM(Misslm)

frequentist_model_table(MissLMERresults, MisslmConfInt,
 # Replace colons in the names of interactions with times symbols
  interaction_symbol_x = TRUE, 
  
  #Rename Title, for No title replace text with NULL
  caption = "Errors of Commission by Probe Placement and Enjoyment Condition")



participant_avg_missExp1 <- SARTmissFilt %>%
  group_by(subject, mturk_sona) %>%
  summarize(avg_miss = mean(miss),
            sd_miss = sd(miss))

participant_avg_missExp1_stat <- SARTmissFilt %>%
  group_by(mturk_sona) %>%
  summarize(avg_miss = mean(miss),
            sd_miss = sd(miss))

# View the new dataframe
print(participant_avg_missExp1)

missbyMWExp1 <- ggplot(participant_avg_missExp1, aes(x = mturk_sona, y = avg_miss, fill = mturk_sona)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Errors of Commission of No-Go Trials By Participant Pool", 
       x = "Participant Pool", 
       y = "Average Errors of Commission of No-Go Trials",
       fill = "Participant Pool") +
  scale_x_discrete(labels = c("MT" = "MTurk", "SS" = "SONA Systems")) +
  scale_fill_manual(values = c("MT" = "blue", "SS" = "red"),
                    labels = c("MT" = "MTurk", "SS" = "SONA Systems")) +
  guides(fill = FALSE) # Explicitly remove the legend

missbyMWExp1 + stat_summary(fun.data="mean_sdl", mult=1,
                                  geom="crossbar", width=0.2, stackdir='center',
                                  position=position_dodge(.9))
missbyMWExp1 + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
...
participant_avg_faExp1_pres <- SARTfaFilt %>%
  group_by(subject, new_exp_column, mturk_sona) %>%
  summarize(avg_FA = mean(FA),
            sd_FA = sd(FA))

study1FA_sample_sizes <- SARTaccFilt %>% 
  group_by(mturk_sona, new_exp_column) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_FA = max(FA, na.rm = TRUE))

# View the new dataframe
print(participant_avg_faExp1_pres)

FAbyMWExp1_2 <- ggplot(participant_avg_faExp1_pres, aes(x = new_exp_column, y = avg_FA, fill = mturk_sona)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average False Alarms of Go Trials between Probe Condition By \nMTurk and SONA System Participants", 
       x = "Probe Condition", 
       y = "Average False Alarms of Go Trials",
       fill = "Participant Pool") +
  scale_x_discrete(labels = c("EXP1" = "Interrupting Probe", "EXP2" = "Post Probe")) +
  scale_fill_manual(values = c("MT" = "blue", "SS" = "red"),
                    labels = c("MT" = "MTurk", "SS" = "SONA Systems")) + coord_flip()
...
FAbyMWExp1_2 + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study1FA_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 0.2 + 0.25, 
                x = new_exp_column,
                color = mturk_sona),  # Use Probe for the interaction and color
            position = position_dodge(width = 0.9),  
            hjust = 0, vjust = -0.5, size = 5, color = "black", fontface = "bold", show.legend = FALSE)
...
Experiment3TS$MWcatch_response[is.na(Experiment3TS$MWcatch_response)] <- 0
Experiment3TS$MWer_final <- ifelse(Experiment3TS$MWer == "Yes" & Experiment3TS$MWcatch_response >= 51, "MW", "NMW")
Experiment3TS <- Experiment3TS %>%
  rename(MWer_pre = MWer,
         MWer = MWer_final)

Experiment3TS <- Experiment3TS %>% dplyr::select(-MWer_pre)
...
Experiment3TS$MWer <- as.factor(Experiment3TS$MWer)
Experiment3TS$MWer <- relevel(Experiment3TS$MWer, ref = "NMW")
...
RTbyMWExp3TSswitch <- ggplot(participant_avg_latency, aes(x = switch, y = avg_latency, fill = switch)) + 
  ...
  scale_fill_manual(values = c("0" = "blue", "1" = "red"),
                    labels = c("0" = "Non-Switch Trial", "1" = "Switch Trial")) + coord_flip()
...
#For Presentation
study2switch_sample_sizes <- Experiment3TS %>% 
  group_by(switch, MWer) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_latency = max(latency, na.rm = TRUE))

participant_avg_latency <- Experiment3TS %>%
  group_by(subject, switch, MWer) %>%
  summarize(avg_latency = mean(latency),
            sd_latency = sd(latency))

RTbyMWExp3TSswitch <- ggplot(participant_avg_latency, aes(x = switch, y = avg_latency, fill = MWer)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Reaction Time of Task Switching Between Switch Conditions and Mind Wanderer Grouping", 
       x = "Switch", 
       y = "Average Task Switching Reaction Time",
       fill = "Mind Wanderer") +
  scale_x_discrete(labels = c("0" = "Non-Switch Trial", "1" = "Switch Trial")) +
  scale_fill_manual(values = c("Yes" = "blue", "No" = "red"),
                    labels = c("Yes" = "Mind Wanderer", "No" = "Not Mind Wanderer")) + coord_flip()
...
RTbyMWExp3TSswitch + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9))+ geom_text(data = study2switch_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 4000 + 0.5, 
                x = switch,
                color = MWer),  # Use Probe for the interaction and color
            position = position_dodge(width = 0.9),  
            hjust = 0, vjust = -0.5, size = 5, color = "black", fontface = "bold", show.legend = FALSE)
...
TSacc <- glmer(correct~new_cond_column+MWer+congruence+switch+new_cond_column:MWer+switch:MWer+congruence:MWer + switch:congruence + (1|subject), data = Experiment3TS, family = binomial)
summary(TSacc)
exp(confint(TSacc))

TSacc <- lmer(correct~new_cond_column+MWer+congruence+switch+new_cond_column:MWer+switch:MWer+congruence:MWer + switch:congruence + (1|subject), data = Experiment3TS)
summary(TSacc)
...
participant_avg_acc_switch <- Experiment3TS %>%
  group_by(subject, switch, congruence) %>%
  summarize(avg_correct = mean(correct))

# View the new dataframe
print(participant_avg_acc_switch)

ACCbyMWExp3TSswitch <- ggplot(participant_avg_acc_switch, aes(x = switch, y = avg_correct, fill = congruence)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Accuracy of Task Switching Between Switch Trials and Congruency", 
       x = "Switch", 
       y = "Average Task Switching Accuracy",
       fill = "Congruence") +
  scale_x_discrete(labels = c("0" = "Non-Switch Trial", "1" = "Switch Trial")) +
  scale_fill_manual(values = c("1" = "blue", "2" = "red"),
                    labels = c("1" = "Congruent Trial", "2" = "Incongruent Trial"))
...
participant_avg_acc_switch_stat <- Experiment3TS %>%
  group_by(switch, congruence) %>%
  summarize(avg_correct = mean(correct),
            avg_sd = sd(correct))


participant_avg_acc_mwer <- Experiment3TS %>%
  group_by(subject, MWer, congruence) %>%
  summarize(avg_correct = mean(correct))

# View the new dataframe
print(participant_avg_acc_switch)

ACCbyMWExp3TSmwer <- ggplot(participant_avg_acc_mwer, aes(x = congruence, y = avg_correct, fill = MWer)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Accuracy of Task Switching Between Congruent Trials and Levels of Mind Wandering", 
       x = "Congruency", 
       y = "Average Task Switching Accuracy",
       fill = "Levels of Mind Wandering") +
  scale_x_discrete(labels = c("1" = "Congruent", "2" = "Incongruent")) +
  scale_fill_manual(values = c("MW" = "blue", "NMW" = "red"),
                    labels = c("MW" = "High Levels of Mind Wandering", "NMW" = "Low Levels of Mind Wandering"))

ACCbyMWExp3TSmwer + stat_summary(fun.data="mean_sdl", mult=1,
                                  geom="crossbar", width=0.2, stackdir='center',
                                  position=position_dodge(.9))
ACCbyMWExp3TSmwer + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
...
participant_avg_acc_mwer_stat <- Experiment3TS %>%
  group_by(MWer, congruence) %>%
  summarize(avg_correct = mean(correct),
            avg_sd = sd(correct))
...
#For Presentation
study2switchACC_sample_sizes <- Experiment3TS %>% 
  group_by(switch, MWer) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_acc = max(correct, na.rm = TRUE))
...
ACCbyMWExp3TSswitch <- ggplot(participant_avg_acc_switch, aes(x = switch, y = avg_correct, fill = MWer)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Accuracy of Task Switching Between Switch Conditions by Mind Wanderer Grouping", 
       x = "Switch", 
       y = "Average Task Switching Accuracy",
       fill = "Mind Wanderer") +
  scale_x_discrete(labels = c("0" = "Non-Switch Trial", "1" = "Switch Trial")) +
...
ACCbyMWExp3TSswitch + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study2switchACC_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 1 + 0.025, 
                x = switch,
                color = MWer),  # Use Probe for the interaction and color
...
#Congruence
study2congruenceACC_sample_sizes <- Experiment3TS %>% 
  group_by(congruence, MWer) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_acc = max(correct, na.rm = TRUE))

participant_avg_acc_congruent <- Experiment3TS %>%
  group_by(subject, congruence, MWer) %>%
  summarize(avg_correct = mean(correct))

# View the new dataframe
print(participant_avg_acc_congruent)

ACCbyMWExp3TScong <- ggplot(participant_avg_acc_congruent, aes(x = congruence, y = avg_correct, fill = MWer)) +   geom_violin(trim = FALSE) +
  labs(title = "Accuracy of Responses Between Congruent Trials by Mind Wanderer Grouping", 
       x = "Congruence", 
       y = "Average Congruent Trial Accuracy",
       fill = "Mind Wanderer") +
  scale_x_discrete(labels = c("1" = "Congruent", "2" = "Incongruent")) +
...
ACCbyMWExp3TScong + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study2congruenceACC_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 1 + 0.025, 
                x = congruence,
                color = MWer),  # Use Probe for the interaction and color
...
Experiment3SART$MWcatch_response[is.na(Experiment3SART$MWcatch_response)] <- 0
Experiment3SART$MWer_final <- ifelse(Experiment3SART$MWer == "Yes" & Experiment3SART$MWcatch_response >= 51, "MW", "NMW")
Experiment3SART <- Experiment3SART %>%
  rename(MWer_pre = MWer,
         MWer = MWer_final)

Experiment3SART <- Experiment3SART %>% dplyr::select(-MWer_pre)
...
Experiment3SART$MWer <- as.factor(Experiment3SART$MWer)
Experiment3SART$MWer <- relevel(Experiment3SART$MWer, ref = "NMW")
Exp3SARTrtFilt <- Experiment3SART %>% filter(correct == 1)
Exp3SARTrtFilt <- Exp3SARTrtFilt %>% filter(trialtype == "Go")
...
Exp3Missmean <- aggregate(miss ~ subject + new_exp_column + new_cond_column, Exp3SARTmissFilt, mean)
...
participant_avg_rt_SART_stats <- Exp3SARTrtFilt %>%
...
ACCbyMWExp3RTSART <- ggplot(participant_avg_rt_SART, aes(x = new_cond_column, y = avg_latency, fill = MWer)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Latency of Responses for Go Trials by Levels of Mind Wanderer and Enjoyment Condition", 
       x = "Enjoyment Condition", 
       y = "Average Latency of Go Trials",
       fill = "Levels of Mind Wanderer") +
  scale_x_discrete(labels = c("Enjoy" = "Enjoy", "NotEnjoy" = "Not Enjoy")) +
  scale_fill_manual(values = c("MW" = "blue", "NMW" = "red"),
                    labels = c("MW" = "High Level of Mind Wandering", "NMW" = "Low Level of Mind Wandering"))
...
#For Presentation
study2sartRT_sample_sizes <- Exp3SARTrtFilt %>% 
  group_by(new_cond_column, MWer) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_latency = max(latency, na.rm = TRUE))
...
ACCbyMWExp3RTSART + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study2sartRT_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 900 + 0.25, 
                x = new_cond_column,
                color = MWer),  # Use Probe for the interaction and color
...
Exp3ACClm <- glmer(correct~new_cond_column * MWer + (1 | subject), data=Exp3SARTaccFilt, family = binomial)
summary(Exp3ACClm)
exp(confint(Exp3ACClm))
...
ACCbyMWExp3ACCSART <- ggplot(participant_avg_acc_SART, aes(x = new_cond_column, y = avg_accuracy, fill = MWer)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Accuracy of Responses for No Go Trials by Levels of Mind Wandering \nand Enjoyment Condition", 
       x = "Enjoyment Condition", 
       y = "Average Accuracy of No Go Trials",
       fill = "Levels of Mind Wandering") +
  scale_x_discrete(labels = c("Enjoy" = "Enjoy", "NotEnjoy" = "Not Enjoy")) +
...
#For Presentation
study2sartACC_sample_sizes <- Exp3SARTaccFilt %>% 
  group_by(new_cond_column, MWer) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_acc = max(correct, na.rm = TRUE))
...
Exp3FAlm <- glmer(FA~new_cond_column * MWer + (1 | subject), data=Exp3SARTfaFilt, family = binomial)
summary(Exp3FAlm)
tidy(Exp3FAlm,conf.int=TRUE,exponentiate=TRUE,effects="fixed")
...
ACCbyMWExp3FASART <- ggplot(participant_avg_fa_SART, aes(x = new_cond_column, y = avg_fa, fill = MWer)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Rate of Errors of Omission by Levels of Mind Wandering \nand Enjoyment Condition", 
       x = "Enjoyment Condition", 
       y = "Average Rate of Errors of Omission",
       fill = "Levels of Mind Wandering") +
  scale_x_discrete(labels = c("Enjoy" = "Enjoy", "NotEnjoy" = "Not Enjoy")) +
  scale_fill_manual(values = c("MW" = "blue", "NMW" = "red"),
                    labels = c("MW" = "High Levels of Mind Wandering", "NMW" = "Low Levels of Mind Wandering"))
...
Exp3misslm <- glmer(miss~new_cond_column * MWer + (1 | subject), data=Exp3SARTmissFilt, family = binomial)
summary(Exp3misslm)
tidy(Exp3misslm,conf.int=TRUE,exponentiate=TRUE,effects="fixed")



participant_avg_miss_SART <- Exp3SARTmissFilt %>%
  group_by(subject, new_cond_column, MWer) %>%
  summarize(avg_miss = mean(miss))

# View the new dataframe
print(participant_avg_miss_SART)

ACCbyMWExp3missSART <- ggplot(participant_avg_miss_SART, aes(x = new_cond_column, y = avg_miss, fill = MWer)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Rate of Errors of Commission by Levels of Mind Wandering \nand Enjoyment Condition", 
       x = "Enjoyment Condition", 
       y = "Average Rate of Errors of Commission",
       fill = "Levels of Mind Wandering") +
  scale_x_discrete(labels = c("Enjoy" = "Enjoy", "NotEnjoy" = "Not Enjoy")) +
  scale_fill_manual(values = c("MW" = "blue", "NMW" = "red"),
                    labels = c("MW" = "High Levels of Mind Wandering", "NMW" = "Low Levels of Mind Wandering"))

ACCbyMWExp3missSART + stat_summary(fun.data="mean_sdl", mult=1,
                                  geom="crossbar", width=0.2, stackdir='center',
                                  position=position_dodge(.9))
ACCbyMWExp3missSART + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
...
#For Presentation
study2sartFA_sample_sizes <- Exp3SARTfaFilt %>% 
  group_by(new_cond_column, MWer) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_fa = max(FA, na.rm = TRUE))
...
ACCbyMWExp3FASART + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study2sartFA_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 0.2 + 0.025, 
                x = new_cond_column,
                color = MWer),  # Use Probe for the interaction and color
...
Exp1WMinfo <- exp1exp2Analysis2 %>% dplyr::select(subject, MWer)
Exp3TSinfo <- Experiment3TS %>% dplyr::select(subject, MWer)
Exp3SARTinfo <- Experiment3SART %>% dplyr::select(subject, MWer)
MWerResultsData <- rbind(Exp1WMinfo, Exp3TSinfo, Exp3SARTinfo)
MWerResultsData <- MWerResultsData[!duplicated(MWerResultsData$subject), ]
...
SurveyAnalysis$MWer <- relevel(SurveyAnalysis$MWer, ref = "MW")
...
PostSurveys$MWer <- relevel(PostSurveys$MWer, ref = "MW")
...
ggplot(PostSurveys, aes(x = paid_response, fill = MWer)) +
  geom_density(alpha=0.2) +
  scale_fill_manual(values = c("High Levels of Mind Wandering" = "red", "Low Levels of Mind Wandering" = "blue"))

ggplot(PostSurveys, aes(x = paid_response, fill = MWer)) +
  geom_density(alpha = 0.2) +
  labs(fill = "Mind Wandering Levels") +
  scale_fill_manual(values = c("blue", "red"),
                    breaks = c("MW", "NMW"),
                    labels = c("High Levels of Mind Wandering", "Low Levels of Mind Wandering"))
...
EFAanalysis$MWcatch_response[is.na(EFAanalysis$MWcatch_response)] <- 0
EFAanalysis$MW <- ifelse(EFAanalysis$MWer == "Yes" & EFAanalysis$MWcatch_response >= 51, "MW", "NMW")
...
EXP1PostAnalysis <- EXP1PostAnalysis %>%
  mutate(Probe = dplyr::recode(new_exp_column, 
                                  EXP1 = 'IP', 
                                  EXP2 = 'PP'))

# EXP1PostAnalysis$Task <- recode(EXP1PostAnalysis$new_exp_column, "'EXP1' = 'SART'; 'EXP2' = 'SART'; 'EXP3-SART' = 'SART'; 'EXP3-TS' = 'TS'", T, F)

EXP1PostAnalysis$Task <- dplyr::recode(EXP1PostAnalysis$new_exp_column,
                                'EXP1' = 'SART',
                                'EXP2' = 'SART',
                                'EXP3-SART' = 'SART',
                                'EXP3-TS' = 'TS')
...
EXP1PostAnalysis$MW <- dplyr::recode(EXP1PostAnalysis$MWer, 
                               'MW' = 'MW', 
                               'NMW' = 'NMW')
...
participant_avg_paid_stat <- EXP1PostAnalysis %>%
     group_by(Probe, MW) %>%
     summarize(avg_pay = mean(paid_response),
               sd_pay = sd(paid_response))
...
Exp1Paidplot <- ggplot(EXP1PostAnalysis, aes(x = Probe, y = paid_response, fill = MW)) + 
  geom_violin(trim = FALSE) +
  ...
  scale_fill_manual(values = c("MW" = "blue", "NMW" = "red"),
                    labels = c("MW" = "Interrupting Probe", "PP" = "Post Probe")) 
...
Exp1Paidplot1 + stat_summary(fun.data=mean_sdl, mult=1,
                             geom="pointrange", color="black", stackdir='center',
                             position=position_dodge(.9)) + geom_text(data = study1paid_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 125 + 0.5, 
                x = Probe,
                color = MW),  # Use Probe for the interaction and color
            position = position_dodge(width = 0.9),  
            hjust = 0, vjust = -0.5, size = 5, color = "black", fontface = "bold", show.legend = FALSE)
...
EXP3PostAnalysis$Probe <- dplyr::recode(EXP3PostAnalysis$new_exp_column,'EXP3-SART' = 'PP', 'EXP3-TS' = 'PP')
EXP3PostAnalysis$Task <- dplyr::recode(EXP3PostAnalysis$new_exp_column, 'EXP3-SART' = 'SART', 'EXP3-TS' = 'TS')
table(EXP3PostAnalysis$Probe, EXP3PostAnalysis$Task)

EXP3PostAnalysis$MW <- dplyr::recode(EXP3PostAnalysis$MWer, 'MW' = 'MW', 'NMW' = 'NMW')
...
PaidMWlmProbeEXP3 <- lm(paid_response~MW+Task+new_cond_:Task, data = EXP3PostAnalysis, contrasts = list(mturk_sona = "contr.sum", Task = "contr.sum", MW = "contr.sum"))
summary(PaidMWlmProbeEXP3)
confint(PaidMWlmProbeEXP3)

PaidMWlmProbeEXP3 <- lm(paid_response~MW+Task+MW:Task, data = EXP3PostAnalysis, contrasts = list(Task = "contr.sum", MW = "contr.sum"))
summary(PaidMWlmProbeEXP3)
confint(PaidMWlmProbeEXP3)
...
Exp3Paidplot <- ggplot(EXP3PostAnalysis, aes(x = MW, y = paid_response, fill = MW)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Amount Required to be Paid to Re-enage by Levels of Mind Wandering", 
       x = "Levels of Mind Wandering", 
       y = "Average Amount of Payment Required to Re-engage",
       fill = "Levels of Mind Wandering") +
  scale_x_discrete(labels = c("MW" = "High Levels of Mind Wandering", "NMW" = "Low Levels of Mind Wandering")) +
  scale_fill_manual(values = c("MW" = "blue", "NMW" = "red"),
                    labels = c("MW" = "High Levels of Mind Wandering", "NMW" = "Low Levels of Mind Wandering")) +
  guides(fill = FALSE)
...
Exp3Paidplot + stat_summary(fun.data=mean_sdl, mult=1,
 ...
 participant_avg_paid_exp3_stats <- EXP3PostAnalysis %>%
     group_by(Task) %>%
     summarize(avg_pay = mean(paid_response),
               sd_pay = sd(paid_response))
...
EFAanalysis$Probe <- dplyr::recode(EFAanalysis$new_exp_column, 'EXP1' = 'IP', 'EXP2' = 'PP', 'EXP3-SART' = 'PP', 'EXP3-TS' = 'PP')
EFAanalysis$Task <- dplyr::recode(EFAanalysis$new_exp_column, 'EXP1' = 'SART', 'EXP2' = 'SART', 'EXP3-SART' = 'SART', 'EXP3-TS' = 'TS')
table(EFAanalysis$Probe, EFAanalysis$Task)

EFAanalysis$MW <- dplyr::recode(EFAanalysis$MWer, 'MW' = 'MW', 'NMW' = 'NMW')

participant_efa_posAff <- EFAanalysis %>%
   group_by(Probe, MW) %>%
   summarize(avg_m1 = mean(MR1),
             sdm1 = sd(MR1))
...
MR1plot <- ggplot(EFAanalysis, aes(x = Probe, y = MR1, fill = MW)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Positive Affect Factor Scores by Probe Condition and Levels of Mind Wandering", 
       x = "Probe Condition", 
...
sample_sizes <- EFAanalysis %>% 
  group_by(MW, Probe) %>% 
  summarise(sample_size = n(), 
            max_MR1 = max(MR1, na.rm = TRUE),
            max_MR3 = max(MR3, na.rm = TRUE),
            max_MR5 = max(MR5, na.rm = TRUE),
            max_PR = max(paid_response, na.rm = TRUE))

MR1plot <- ggplot(EFAanalysis, aes(x = MW, y = MR1, fill = Probe)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Positive Affect Factor Scores by Mind Wanderer and Probe Condition", 
       x = "Mind Wanderer", 
       y = "Average Positive Affect Factor Scores",
       fill = "Probe Condition") +
  scale_x_discrete(labels = c("MW" = "Mind Wanderer", "NMW" = "Not Mind Wanderer")) +
  scale_fill_manual(values = c("IP" = "blue", "PP" = "red"),
                    labels = c("IP" = "Interrupting Probe", "PP" = "Post Probe")) + coord_flip()
...
MR1plot + stat_summary(fun.data=mean_sdl, mult=1, 
                 geom="pointrange", color="black", stackdir = 'center',
                 position=position_dodge(.9)) + geom_text(data = sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = max_MR1 + 0.5, 
                x = MW,
                color = Probe),  # Use Probe for the interaction and color
            position = position_dodge(width = 0.9),  
            hjust = 0, vjust = -0.5, size = 5, color = "black", fontface = "bold", show.legend = FALSE)
...
MR1plot <- ggplot(EFAanalysis, aes(x = Probe, y = MR1, fill = MW)) + 
  ...
  scale_x_discrete(labels = c("IP" = "Interrupting Probe", "PP" = "Post Probe")) +
  scale_fill_manual(values = c("MW" = "blue", "NMW" = "red"),
                    labels = c("MW" = "Mind Wanderer", "NMW" = "Not Mind Wanderer")) + coord_flip()
...
MR3plot <- ggplot(EFAanalysis, aes(x = Probe, y = MR3, fill = Probe)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average GTQ Factor Scores by Probe Condition and Mind Wanderer", 
       x = "Probe Condition", 
...
MR5plot <- ggplot(EFAanalysis, aes(x = MW, y = MR5, fill = MW)) + 
  geom_violin(trim = FALSE) +
  labs(title = "Average Stress Factor Scores by Probe Condition and Mind Wanderer", 
       x = "Levels of Mind Wandering", 
       y = "Average Stress Factor Scores",
       fill = "Levels of Mind Wandering") +
  scale_x_discrete(labels = c("MW" = "High Levels of Mind Wandering", "NMW" = "Low Levels of Mind Wandering")) +
...
study3paid_sample_sizes <- EFAanalysis %>% 
  group_by(Probe, MW) %>% 
  summarise(sample_size = n_distinct(subject), 
            max_paid = max(paid_response, na.rm = TRUE))
...
PaidRespS3 + stat_summary(fun.data=mean_sdl, mult=1,
                                  geom="pointrange", color="black", stackdir='center',
                                  position=position_dodge(.9)) + geom_text(data = study3paid_sample_sizes, 
            aes(label = paste("n =", sample_size), 
                y = 125 + 0.5, 
                x = Probe,
                color = MW),  # Use Probe for the interaction and color
...
participant_avg_paid_efa_stats <- EFAanalysis %>%
     group_by(Probe, MW) %>%
     summarize(avg_pay = mean(paid_response),
               sd_pay = sd(paid_response))
