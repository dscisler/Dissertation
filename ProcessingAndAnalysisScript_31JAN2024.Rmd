...
PaidMWlmProbeEXP1_2 <- lm(paid_response~new_exp_column*MWer, data = EXP1PostAnalysis, contrasts = list(new_exp_column = "contr.sum", MWer = "contr.sum"))
...
ggplot(PaidMWlmProbeEXP1_2_data, aes(x = new_exp_column, y = mean_value, fill = MWer)) +
  geom_bar(stat = "summary", fun = "mean", position = "dodge") +
  scale_fill_manual(values = c("Yes" = "blue", "No" = "red")) +
  geom_errorbar(aes(ymin = mean_value - ci, ymax = mean_value + ci),
                position = position_dodge(1), width = 0.2) +
  labs(title = "Average Reported Amount to be Paid to Re-engage With the Task by Probe Condition\nand Mind Wanderer", x = "Experimental Condition", y = "Average Reported Amount to be Paid", fill = "Mind Wanderer") +
  scale_x_discrete(labels = c("EXP1" = "Interrupting Probe",
                              "EXP2" = "Post Probe"))
